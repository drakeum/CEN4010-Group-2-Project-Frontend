...
const AdminProperties = () => {
    ...
    async function getUsername(uid) {       //need a way of getting this to run after the properties.map() but not in it???? h o w
        const uidNum = Number(uid);
        const response = await fetch(`https://cen4010-pms-backend.herokuapp.com/api/admin/getUserById/${uidNum}`, {
            method: "GET",
            headers: ({ 'Authorization': accessToken })
        });
        const data = await response.json();
                                    ...
                                    <Grid item xs>
                                        <Stack direction="row">
                                            ...
                                            <Typography gutterBottom variant="subtitle1" component="div" sx={{ ml: 1 }}>
                                                ID: {property.id}, Name: {property.name}, Owner Name: {async () => await getUsername(property.ownerAccountID)}
                                            </Typography>
                                        </Stack>
